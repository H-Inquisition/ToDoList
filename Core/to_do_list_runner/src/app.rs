...
pub async fn run(address: &str) -> Result<()> {
    ...
    let router = Router::new()
        ...
        .route("/update_task_status", post(api::commands::update_task_status_command)).with_state(state.clone())
        .route("/update_task_title", post(api::commands::update_task_title_command)).with_state(state.clone())
        .route("/update_task_priority", post(api::commands::update_task_priority_command)).with_state(state.clone())
...
fn log_error(err: &Error) -> Result<()> {
    let mut file = OpenOptions::new().write(true).append(true).open("./error_log.txt").map_err(|_| Error::OpenFile)?;
